...
fn run() -> Result<()> {
    ...
    if target.os == "windows" && target.env == "gnu" {
        let prebuilt = current_dir()
            .unwrap()
            .join("prebuilt")
            .join("bindings_windows_x86_64.rs");
        fs::copy(&prebuilt, &bindings_out).map_err(|e| {
            format!(
                "Unable to copy prebuilt bindings from {prebuilt:?} to {bindings_out:?}: {e}"
            )
        })?;
        println!("cargo:warning=Using prebuilt mupdf bindings for windows-gnu target");
    } else {
        generate_bindings(&target, &bindings_out, sysroot)
